...
class WebhookPublisherThreaded:
    ...
    def close(self, drain_sec: float = 1.0) -> None:
        """
        Best-effort drain for up to drain_sec, then exit quickly.
        """
        if not self.enabled:
            return

        t_end = time.time() + max(0.0, float(drain_sec))
        # Let the worker know we want to stop, but it will still try to flush until timeout.
        self._stop.set()
        while time.time() < t_end and (not self._q.empty()):
            time.sleep(0.01)

        self._thread.join(timeout=max(0.0, float(drain_sec)))
        ...
        with httpx.Client(timeout=self.timeout_sec) as client:
            while True:
                ...
                try:
                    ...
                    _ = r.status_code  # swallow errors
                except Exception:
                    pass
                finally:
                    try:
...
class MarketMonitor:
    ...
    async def on_quote(self, q: Any) -> None:
        self.quote_cache.update_from_stream(q)

    async def on_1m_bar(self, bar: Any) -> None:
    ...
    def run(self) -> None:
        ...
        if self.regime_symbol:
            ...
            print("Regime benchmark symbol: AUTO (select highest CONFIRMED at 30m)")
